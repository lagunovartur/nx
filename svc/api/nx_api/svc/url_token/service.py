...
class UrlTokenSvc(IUrlTokenSvc):
    ...
    async def encode(self, payload: T) -> str:
        ...
        return self._url_serializer.dumps(url_payload.model_dump(), salt=self._action_salt(action))
    ...
    async def decode(self, token: str, payload_type: Type[T], ttl: int = 15) -> UrlPayload[T]:
        ...
        try:
            decoded = self._url_serializer.loads(token, max_age=ttl*60, salt=self._action_salt(action))
