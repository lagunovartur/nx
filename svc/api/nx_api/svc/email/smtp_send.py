...
class SmtpSend(ISendMail):
    ...
    async def __call__(
        self, subject: str, template: str, data: dict, recipients: list[Email]
    ) -> dict[Email, Exception]:
        body = self._templates.get_template(template).render(data)
        tasks = [
            create_task(self._smtp.send_message(self._msg(subject, body, recipient)))
            for recipient in recipients
        ]
        send_result = zip(await gather(*tasks, return_exceptions=True), recipients)

        send_errors = {
            recipient: task_result
            for task_result, recipient in send_result
